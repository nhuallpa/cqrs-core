...
public abstract class AggregateRoot {
    ...
    protected void applyChanges(BaseEvent event, Boolean isNewEvent) {
        try {
            ...
            logger.log(Level.WARNING, "The apply method was not found in the aggragate for {0}", event.getClass().getName());
        } catch (Exception e) {
            logger.log(Level.SEVERE, "Error applyng event to aggregate", e);
        } finally {
            if (isNewEvent) {
                changes.add(event);
            }
